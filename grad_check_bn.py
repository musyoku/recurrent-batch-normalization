...
def _batch_normalization(expander, gamma, x, mean, var):
	mean = mean[expander]
	std = numpy.sqrt(var)[expander]
	y_expect = (gamma[expander] * (x - mean) / std)
	return y_expect
...
class TestBatchNormalization(unittest.TestCase):
	...
	def check_backward(self, args, y_grad):
		gradient_check.check_backward(
			bnlstm.NoBetaBatchNormalizationFunction(eps=self.eps),
...
class TestFixedBatchNormalization(unittest.TestCase):
	...
	def setUp(self):
		...
		self.args = [self.x, self.gamma, self.mean, self.var]
	...
	def check_backward(self, args, y_grad):
		gradient_check.check_backward(
			bnlstm.NoBetaBatchNormalizationFunction(eps=self.eps),args, 
			y_grad, eps=1e-2, rtol=1e-3, atol=1e-4)
